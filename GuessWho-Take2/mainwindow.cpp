...
MainWindow::MainWindow(){
    ...
    guessWho = new QPushButton(tr("Insert Guess Who push button here"));
...
void MainWindow::createCharGroupBox(){
    ...
    for (int i = 0; i < NumGridRows; i++) {
        for (int j = 0; j < NumGridCols; j++){
            ...
            characters[i][j]->setFixedSize(164,134);
...
void MainWindow::createQuestionGroupBox(){
    ...
    SkinColor = new QPushButton(tr("&Skin Color"));
    SkinColor->setFixedSize(172,48);
    SkinColor->setFont(QFont("MS Shell Dlg 2", 11, QFont::Bold));
    ...
    Accessories = new QPushButton(tr("&Accessories"));
    Accessories->setFixedSize(172,48);
    Accessories->setFont(QFont("MS Shell Dlg 2", 11, QFont::Bold));
    Accessories->setStyleSheet("QPushButton{background-image:url(:/program/images/Default.png); color: white; border-width: 3px; border-color: #181D1F; border-style: outset; border-radius: 7;}"
                               "QPushButton:hover{background-image:url(:/program/images/Hover.png);}"
                               "QPushButton:pressed{background-image:url(:/program/images/Clicked.png);}");

    // Sets up a 2x3 grid with each button occupying a space
    quesLayout->addWidget(SkinColor, 0, 0, 1, 1);
    quesLayout->addWidget(Gender, 0, 1, 1, 1);
    quesLayout->addWidget(EyeColor, 0, 2, 1, 1);
    quesLayout->addWidget(Hair, 1, 0, 1, 1);
    quesLayout->addWidget(FacialHair, 1, 1, 1, 1);
    quesLayout->addWidget(Accessories, 1, 2, 1, 1);
